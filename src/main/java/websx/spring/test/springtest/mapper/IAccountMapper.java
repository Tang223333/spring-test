...
public interface IAccountMapper {
    ...
    @Insert("insert into account(uid,name,password,email,phone,time,type,logo) values" +
            "(#{uid},#{name},#{password},#{email},#{phone},#{time},#{type},#{logo});")
    void saveAccount(Account account);

    @Update("update account set name=#{name},password=#{password},email=#{email},phone=#{phone},time=#{time},type=#{type},logo=#{logo} where id=#{id};")
